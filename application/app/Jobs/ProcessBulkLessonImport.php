...
class ProcessBulkLessonImport implements ShouldQueue
{
     ...
     */
    public function __construct(array $requestData, int $adminId, string $runId)
    {
    ...
    public function handle(): void
    {
        ...
        if (!$apiKey) {
            Log::error('bulk_lessons.youtube_import.error', [
                'run_id' => $this->runId,
                'admin_id' => $this->adminId,
                'message' => 'Missing YOUTUBE_API_KEY in .env',
            ]);
            return;
        }

        $purifier = new \HTMLPurifier();
        $importComments = (int) ($this->requestData['import_comments'] ?? 1) === 1;
        $commentsLimit = (int) ($this->requestData['comments_limit'] ?? 20);

        try {
            $source = $this->detectYouTubeSource($this->requestData['youtube_url']);
            if (!$source) {
                Log::error('bulk_lessons.youtube_import.error', [
                    'run_id' => $this->runId,
                    'admin_id' => $this->adminId,
            ...
            if (!in_array($source['type'], ['video', 'playlist', 'channel'], true)) {
                $channelId = $this->resolveChannelIdFromSource($source, $apiKey, $this->runId);
                if (!$channelId) {
                    Log::error('bulk_lessons.youtube_import.error', [
                        'run_id' => $this->runId,
                        'admin_id' => $this->adminId,
                        'message' => 'Unable to resolve the YouTube channel from the provided URL',
                    ]);
                    return;
                }
                $source = ['type' => 'channel', 'channel_id' => $channelId];
            }

            Log::info('bulk_lessons.youtube_import.start', [
                'run_id' => $this->runId,
                'admin_id' => $this->adminId,
                'course_id' => (int) $this->requestData['course_id'],
                'level' => (int) $this->requestData['level'],
                'value' => (int) $this->requestData['value'],
                'source' => $source,
                'import_comments' => $importComments,
                'comments_limit' => $commentsLimit,
            ]);

            // Create admin notification for job start
            $course = Course::find((int) $this->requestData['course_id']);
            $courseTitle = $course ? $course->title : 'Unknown Course';

            $adminNotification = new AdminNotification();
            $adminNotification->user_id = $this->adminId;
            $adminNotification->title = "Bulk lesson import started for course: {$courseTitle}";
            $adminNotification->click_url = route('admin.lesson.courses', $this->requestData['course_id']);
            $adminNotification->read_status = 0;
            ...
            if ($source['type'] === 'video') {
                $video = $this->fetchVideoById($source['video_id'], $apiKey, $this->runId);
                if (!$video) {
                    Log::error('bulk_lessons.youtube_import.error', [
                        'run_id' => $this->runId,
                        'admin_id' => $this->adminId,
                        'message' => 'No video found for the provided URL',
                    ]);
                    return;
                }
                $ok = $this->createLessonFromVideo($video, $this->requestData, $purifier, $apiKey, $importComments, $commentsLimit, $this->runId);
                $ok ? $created++ : $failed++;
            } elseif ($source['type'] === 'playlist') {
                $importResult = $this->importFromPlaylist($source['playlist_id'], $this->requestData, $purifier, $apiKey, $importComments, $commentsLimit, $this->runId);
                $created += $importResult['created'];
                $failed += $importResult['failed'];
                $stoppedEarly = $importResult['stopped_early'];
                $stopReason = $importResult['stop_reason'];
            } elseif ($source['type'] === 'channel') {
                $importResult = $this->importFromChannel($source['channel_id'], $this->requestData, $purifier, $apiKey, $importComments, $commentsLimit, $this->runId);
                $created += $importResult['created'];
                $failed += $importResult['failed'];
                $stoppedEarly = $importResult['stopped_early'];
                $stopReason = $importResult['stop_reason'];
            ...
            Log::info('bulk_lessons.youtube_import.done', [
                'run_id' => $this->runId,
                'created' => $created,
                'failed' => $failed,
            ...
            // Create admin notification for job completion
            $course = Course::find((int) $this->requestData['course_id']);
            $courseTitle = $course ? $course->title : 'Unknown Course';
            $statusMessage = $stoppedEarly ? " (Stopped early: {$stopReason})" : "";

            $adminNotification = new AdminNotification();
            $adminNotification->user_id = $this->adminId;
            $adminNotification->title = "Bulk lesson import completed for course: {$courseTitle} - {$created} created, {$failed} failed{$statusMessage}";
            $adminNotification->click_url = route('admin.lesson.courses', $this->requestData['course_id']);
            $adminNotification->read_status = 0;
            $adminNotification->save();
            ...
            $adminNotification = new AdminNotification();
            $adminNotification->user_id = $this->adminId;
            $adminNotification->title = "Bulk lesson import failed for course: {$courseTitle} - Error: " . substr($e->getMessage(), 0, 100);
            $adminNotification->click_url = route('admin.lesson.courses', $this->requestData['course_id']);
            $adminNotification->read_status = 0;
            $adminNotification->save();
    ...
    {
        // Create admin notification for final job failure after all retries
        $course = Course::find((int) $this->requestData['course_id']);
        $courseTitle = $course ? $course->title : 'Unknown Course';

        $adminNotification = new AdminNotification();
        $adminNotification->user_id = $this->adminId;
        $adminNotification->title = "Bulk lesson import permanently failed for course: {$courseTitle} - Final attempt exhausted";
        $adminNotification->click_url = route('admin.lesson.courses', $this->requestData['course_id']);
        $adminNotification->read_status = 0;
        $adminNotification->save();

        Log::error('bulk_lessons.youtube_import.permanent_failure', [
            'run_id' => $this->runId,
            'admin_id' => $this->adminId,
            'course_id' => (int) $this->requestData['course_id'],
    ...
    private function importFromPlaylist(string $playlistId, array $requestData, \HTMLPurifier $purifier, string $apiKey, bool $importComments, int $commentsLimit, string $runId): array
    {
        $pageToken = null;
        $created = 0;
        $failed = 0;
        ...
        do {
            ...
            foreach (($json['items'] ?? []) as $item) {
                ...
                $ok = $this->createLessonFromVideo($video, $requestData, $purifier, $apiKey, $importComments, $commentsLimit, $runId);
                $ok ? $created++ : $failed++;
            }
    ...
    private function importFromChannel(string $channelId, array $requestData, \HTMLPurifier $purifier, string $apiKey, bool $importComments, int $commentsLimit, string $runId): array
    {
        ...
        return $this->importFromPlaylist($uploads, $requestData, $purifier, $apiKey, $importComments, $commentsLimit, $runId);
    }
    ...
    private function createLessonFromVideo(array $video, array $requestData, \HTMLPurifier $purifier, string $apiKey, bool $importComments, int $commentsLimit, string $runId): bool
    {
        $videoId = $video['video_id'] ?? null;
        try {
            $lesson = new Lesson();
            $lesson->title = (string) ($video['title'] ?? $videoId ?? 'Untitled');
            $lesson->owner_id = $this->adminId;
            $lesson->owner_type = 1;
            ...
            $lesson->value = $requestData['value'];
