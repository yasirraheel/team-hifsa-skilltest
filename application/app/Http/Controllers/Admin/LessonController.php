...
class LessonController extends Controller
{
    ...
    function create()
    {
        ...
        return view('admin.lessons.create', compact('pageTitle', 'courses'));
    }
    ...
    function store(Request $request)
    {
        ...
        $request->validate([
            ...
            'title' => [
                ...
                function (string $attribute, mixed $value, Closure $fail) use ($lesson) {
                    $existing_lesson_names = $lesson->adminOwner()->pluck('title')->map(function ($name) {
        ...
        if ($request->preview_video == 3 && $request->start_time && $request->start_time > now()) {
            ...
            if ($allNotNull) {
                ...
                if (!($admin->zoom_account_id) && !($admin->zoom_client_id) && !($admin->zoom_secret_id)) {
    ...
    function edit($id)
    {
        ...
        return view('admin.lessons.edit', compact('pageTitle', 'categories', 'lesson', 'course', 'upload_video'));
    }
        ...
        $request->validate([
            ...
            'title' => [
                ...
                function (string $attribute, mixed $value, Closure $fail) use ($lessonObject, $id) {
                    $existing_lesson_names = $lessonObject->adminOwner()->whereNot('id', $id)->pluck('title')->map(function ($name) {
                        return strtolower($name);
                    });
                    if ($existing_lesson_names->contains(strtolower($value))) {
    ...
    public function editVideoUploadDelete(Request $request)
    {
        ...
        if (!session()->get('videoUpload')) {
            $lesson = Lesson::adminOwner()->where('id', $request->id)->first();
